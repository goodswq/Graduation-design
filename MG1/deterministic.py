...
def simulate(customer_num, lmd, mu):
    """
    模拟排队（确定型分布）
...
def theoretical(lmd, mu):
    ...
    """
    # 确定性分布
    Ws = (2*mu - lmd) / (2*mu*(mu - lmd))
    Wq = lmd / (2*mu*(mu - lmd))
    Ls = (2*mu*lmd - lmd*lmd) / (2*mu*mu - 2*mu*lmd)
    Lq = lmd *lmd / (2*mu*(mu - lmd))
...
for i in range(repeat):
    temp = simulate(customer_num, lmd, mu)
    for i in range(4):
...
theoretical_list = theoretical(lmd, mu)
